    ...
    switch (type) {
        case "checkbox":
            ...
            if (checked.length > 0) {
                ...
                if ($(`#question_${index} input[type=text]`)) {
        ...
        case "text":
            ...
            if (element.value != "") {
                ...
                if ($(`#question_${index} input[type=checkbox]`)) {
...
export function load_answers(index) {
    ...
    if ($(`#question_${index} input[type=text]`) && $(`#question_${index} input[type=checkbox]`)) {
        // Check if the stored value is not empty.
        if (answers[$(`#question_${index} input[type=text]`).name + "_text"] != undefined) {
            // Fill in the stored value in the textfield.
            $(`#question_${index} input[type=text]`).value = answers[$(`#question_${index} input[type=text]`).name + "_text"]
        }

        // Check if the stored value is not empty.
        if (answers[$(`#question_${index} input[type=checkbox]`).name + "_checkbox"] != undefined) {
            $$(`#question_${index} input[type=checkbox]`).forEach(element => {
                answers[$(`#question_${index} input[type=checkbox]`).name + "_checkbox"].forEach(checkbox => {
        ...
        if ($(`#question_${index} input[type=text]`)) {
            // Check if the stored value is not empty.
            if (answers[$(`#question_${index} input[type=text]`).name] != undefined) {
                // Fill in the stored value in the textfield.
                $(`#question_${index} input[type=text]`).value = answers[$(`#question_${index} input[type=text]`).name]
            }
        } else if ($(`#question_${index} input[type=radio]`)) {
            // Check if the stored value is not empty.
            if (answers[$(`#question_${index} input[type=radio]`).name] != undefined) {
                // Check the stored radio button.
                $(`#${answers[$(`#question_${index} input[type=radio]`).name]}`).checked = true
            }
        } else if ($(`#question_${index} input[type=checkbox]`)) {
            // Check if the stored value is not empty.
            if (answers[$(`#question_${index} input[type=checkbox]`).name] != undefined) {
                $$(`#question_${index} input[type=checkbox]`).forEach(element => {
                    answers[$(`#question_${index} input[type=checkbox]`).name].forEach(checkbox => {
                        // Check the stored checkboxes.
                        if (checkbox == element.id) {
                            element.checked = true
                        }
